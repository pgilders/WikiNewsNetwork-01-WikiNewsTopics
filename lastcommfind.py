...
# =============================================================================
        ...
        for n in range(tlen):
            ...
            sel = sel[sel['target'] != 'Main Page']
            tuples = [tuple(x) for x in sel.values]
            glist.append(igraph.Graph.TupleList(
                tuples, directed=False, edge_attrs=['weight']
            ).simplify(combine_edges='first'))
            glist[n].vs["slice"] = n
...
#        del tuples
        ...
        membdf = pd.concat(
            [pd.Series(x, index=glist[n].vs['name'])
             for n, x in enumerate(membership)], axis=1, sort=True)
...
# %%
donesamp = set([x.replace('active_data/robusttest21',
                          '/Volumes/Samsung_T3/evtest')
    ...
    try:
        ...
        for q in [0, 0.5, 0.9]:
            out[q] = commmdectFRparallel(
                [x, evlsn.loc[x]], resrange, quantile=q,
                fp='/simtgraph5NN.npz', flat=True)
        outd[x] = out
    except Exception as ex:
        print(ex)
        # %%
...
# %%
for x in dones:
    if x.replace('active_data/robusttest21',
                 '/Volumes/Samsung_T3/evtest') in outd.keys():
        continue
    out = {}
    for q in [0, 0.5, 0.9]:
        mdfd = {}
...
for q in [0, 0.5, 0.9]:
    plt.plot(amidfd[q].mean(axis=1), label='Mean - %.1f' % q, c=colors[q])
    plt.plot(amidfd[q].median(axis=1), label='Median - %.1f' %
             q, c=colors[q], ls=':')
plt.legend()
plt.xscale('log')
...
# %%
for q in [0, 0.5, 0.9]:
    plt.plot(csdfd[q].mean(axis=1), label='Mean - %.1f' % q, c=colors[q])
    plt.plot(csdfd[q].median(axis=1), label='Median - %.1f' %
             q, c=colors[q], ls=':')
plt.legend()
plt.xscale('log')
plt.xlabel('Resolution')
...
for n in range(0, len(edf), step):
    ...
    try:
        out1 = Parallel(n_jobs=-1, verbose=10)(delayed(cd1)(
            [x, evlsn.loc[x]], res, quantile, fp='/simtgraph5NN.npz')
        ...
        for m, i in enumerate(out2):
            try:
                ...
                for k, v in i[1].items():
                    v.to_hdf(
                        '%s/%s.h5' % (names[m].replace('Samsung_T3/evtest',
                                                       'PGPassport/evtLocal'),
                                      k), key='df')
...
out = Parallel(n_jobs=-1, verbose=10)(delayed(cdflatR)
                                      ([x, evlsn.loc[x]], resrange,
                                       fp='/simtgraph5NN.npz') for x in evsamp)
# %%
amidf = pd.DataFrame()
...
flatadj_D = {x[:-9].replace('active_data/tempevs',
                            '/Volumes/Samsung_T3/evtest'): pd.read_hdf(x, key='df')
...
for m in range(0, len(allev), step):
    print(m)
    out = Parallel(n_jobs=-1, verbose=10)(delayed(cdflatR)([x, evlsn.loc[x]],
                                                           resrange, core_D[x],
...
n = 60000000
print(n, len(combos))
jacsw = Parallel(n_jobs=-1, verbose=3)(delayed(wjac)(x[0], x[1])
...
with open('Downloads/jw/jacsw%.1f.json' % (n/1E6), 'r') as f:
    j2 = json.load(f)
# %%
del combos
jll = []
...
# %%
pd.Series(data, index=pi.columns[1:]).plot(label='AMI')
pd.Series(datc, index=pi.columns[1:]).plot(label='Clusim')
plt.legend()
plt.xscale('log')
plt.title('Partsim')
plt.xlabel('Resolution')
...
for i in meanwj[meanwj['Name'].isna()].index:
    ...
    allarts = pd.concat([DDC['/Volumes/PGPassport/evtLocal/'+x]
                         for x in gjdf[gjdf['comm'] == i].index]).reset_index().groupby('index').sum().sort_values(by=0, ascending=False)
    n = 0
    while True:
        print('Stories')
...
def colourer(x):
    if x == '?':
        return None
    ou = meanwj[meanwj['Name'] == x][0.6065306597126334]
    if len(ou) != 1:
        print(x)
        raise
    if ou.iloc[0] > UQ:
